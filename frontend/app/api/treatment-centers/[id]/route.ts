...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const ok = updateMemoryTreatmentCenter(id, fields);
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { default: getPool } = await import("@/lib/db");
    const pool = getPool();
    await pool.query("DELETE FROM treatment_centers WHERE id = ?", [id]);
    return NextResponse.json({ ok: true, source: "db" });
  } catch {
    // fall back to memory
  }
  const ok = deleteMemoryTreatmentCenter(id);
  return NextResponse.json({ ok });
}
