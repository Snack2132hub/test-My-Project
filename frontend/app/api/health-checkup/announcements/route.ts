...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, source: "memory", data: getMemoryAnnouncements() });
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!title) return NextResponse.json({ ok: false, message: "กรุณาระบุชื่อประกาศ" }, { status: 400 });
