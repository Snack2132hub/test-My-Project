...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { title, category, image_url, content, document_url, deadline_at } = body;
    try {
    ...
    const ok = updateMemoryNews(id, { title, category, image_url, content, document_url, deadline_at: deadline_at || null });
